...
class AttentionBlock(torch.nn.Module):
    ...
    if TYPE_CHECKING:
        def __call__(self,
            ...
            Args:
                ...
                attn_mask (torch.Tensor | None, optional): 注意力遮罩.
    ...
    def __init__(self, config:AttentionBlockConfig):
        ...
        """MLP 块"""
    def forward(
        self, x: torch.Tensor, attn_mask: torch.Tensor, which_cache: Literal["cond", "uncond"] = 'cond'
    ) -> torch.Tensor:
        x = x + self.attention(x, attn_mask, which_cache)
        x = x + self.mlp(x)
        return x
